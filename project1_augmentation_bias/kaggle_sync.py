...
def _render_kernel_runner(config):
    ...
    run_src = (PROJECT_DIR / "run_project1.py").read_text(encoding="utf-8")
    ...
    strategies = config.get("strategies", ["none", "rotation", "crop", "color_jitter", "cutout"])
    strategy_args = " ".join(str(item) for item in strategies)
...
def _render_kernel_metadata(config):
    return {
        ...
        "code_file": "run_project1_kaggle.py",
        "language": "python",
        "kernel_type": "script",
...
def stage_kernel():
    ...
    runner_path = STAGE_DIR / "run_project1_kaggle.py"
...
def download_output():
    config = _load_config()
    _check_kaggle_cli()
    output_dir = PROJECT_DIR / config.get("output_dir", str(DEFAULT_OUTPUT_DIR.name))
...
def main():
    parser = argparse.ArgumentParser(description="Local Kaggle sync helper for Project 1")
    subparsers = parser.add_subparsers(dest="command", required=True)

    subparsers.add_parser("prepare-config", help="Create a working Kaggle config.")
